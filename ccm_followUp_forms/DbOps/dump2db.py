...
from ..utils.custom_exception import ApplicationException
...
class DumpToDB:
    ...
    @staticmethod
    def followup_form_settings(patient_account,gender,practice_code,patient_name):
        try:
            # import pdb
            # pdb.set_trace()
            ...
            TITLE='Monthly Health Assessment Questionnaire'
            DESCRIPTION='As part of your chronic care management program, we kindly ask you to complete this brief questionnaire. Your responses will help us track your health and ensure you receive the best care possible.'
            FORM_CREATION_TIME='00:5:00'
            PROVIDER_REVIEW_TIME='00:5:00'
            CREATED_BY='AI'
            NAME='AI Follow-up Form - '+str(patient_name)
            TYPE= 'Follow up form'
    ...
    def insert_question(df,patient_account,gender,practice_code,uid,patient_name):
        if df.shape[0]>0:
            try:
                PATIENT_FORM_ID=DumpToDB.followup_form_settings(patient_account,gender,practice_code,patient_name)
                df['PATIENT_FORM_ID']=PATIENT_FORM_ID
                df['FORM_BUILDER_ID']=None
                df['MANDATORY']=0
                df.loc[df['TITLE'].str.lower().isin(["por favor, escriba su nombre claramente como su firma.","por favor, escriba su nombre claramente como su firma","please write your name clearly as your signature","por favor escriba su nombre claramente como su firma","please write your name clearly as your signature.","por favor escriba su nombre claramente como su firma."]),'MANDATORY'] = 1
        ...
        if 'response' in js and len(js['response']) > 0:
            for idx, question in enumerate(js['response'], start=1):
                ...
                if 'options' in question:
                    seen=set()
                    col = 1
                    for ix, option in enumerate(question['options'], start=1):
                        if ix>30:
                            break
                        upper_option=option.upper()
                        if upper_option not in seen:
                            seen.add(upper_option)
                            df['Value_' + str(col)] = ['Option ' + str(ix)]
                            col += 1
                            df['Value_' + str(col)] = [option]
                            col += 1
    ...
    def check_form_status(patient_account, uid):
        try:
            ...
            WHERE PATIENT_ACCOUNT = {patient_account} and [TYPE]='Follow up form' and CREATED_BY = 'AI' and isnull(deleted,0)=0 order by created_date desc;
            """
            conn, cursor = DBConnection.live_db()  
